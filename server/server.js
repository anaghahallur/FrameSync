...
const allowedOrigins = [
  ...
  'http://127.0.0.1:5500',
  'http://localhost:5173',
  'https://framesync-frontend.onrender.com', // Explicit Render Frontend
...
app.use((req, res, next) => {
  ...
  // Removed Cross-Origin-Embedder-Policy as it blocks Google and YouTube scripts
  next();
});
  ...
  cors: {
    origin: (origin, callback) => {
      if (!origin || allowedOrigins.includes(origin)) {
        ...
        // Fallback: Mirror the origin specifically to allow credentials
        callback(null, true);
      }
    },
    methods: ['GET', 'POST'],
